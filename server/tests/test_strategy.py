...
def test_dummy_strategy_beats_random(row_inc, col_inc):
    _first_strategy_wins(
        DummyStrategy(row_inc, col_inc),
        RandomStrategy(),
        10
    )
...
def _first_strategy_wins(strategy_a: BaseStrategy, strategy_b: BaseStrategy, max_steps: int):

    def next_move(strategy: BaseStrategy, player: set[int], opponent: set[int], last_step: int):
    ...
    for _ in range(max_steps):
        status, player_a_last_step = next_move(strategy_a, player_a, player_b, player_a_last_step)
        if status == GameStatus.Finished:
            break
        status, player_b_last_step = next_move(strategy_b, player_b, player_a, player_b_last_step)
        if status == GameStatus.Finished:
